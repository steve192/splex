...
FRONTEND_SOURCE_JSON_PATH = ROOT_DIR / "frontend" / "src" / "shared" / "legal" / "openSourceComponents.generated.json"
BACKEND_BUNDLED_FRONTEND_JSON_PATH = BACKEND_DIR / "src" / "splex" / "shared" / "openSourceComponents.generated.json"
...
def serialize_backend_distribution(dist: metadata.Distribution) -> dict:
    ...
    if not homepage and project_urls:
        homepage = project_urls[0].split(",", 1)[-1].strip() if "," in project_urls[0] else project_urls[0]
    ...
    return {
        ...
        "licenseText": read_license_file_candidates(package_root, ["LICENSE", "LICENCE", "COPYING"]),
    }
...
def build_open_source_payload() -> dict:
    backend_components = [serialize_backend_distribution(dist) for dist in dependency_closure(discover_backend_direct_dependencies())]
    ...
    return {
        ...
        "app": {
            ...
            "thirdPartyNotice": (
                ...
                "Their copyright notices, license texts, and NOTICE files must be preserved where required."
            ),
        },
        "sections": [
